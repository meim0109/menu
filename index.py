...
def webhook():
    ...
    if (action == "menuChoice"):
        ...
        info = "您要查詢減肥菜單的時段：" + menu +"且您要查詢減肥菜單的天數：" + time + "，相關資料："+"<br>" 
        ...
        for doc in docs:
            ...
            if menu in dict["time"] && time in dict["date"]:
                    info += "時段：" + dict["time"] + "<br>"
                    info += "天數：" + dict["date"] + "<br>"
                    info += "主食：" + dict["Staple Food"] + "<br>"
                    info += "配餐：" + dict["Nonstaple Food"] + "<br>"
                    info += "飲品：" + dict["beverage"] + "<br>"   
                    info += "水果：" + dict["fruit"] + "<br>" 
